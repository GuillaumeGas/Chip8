...
void OpCls::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	screen->clear();
}
...
void OpRet::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->sp--;
...
void OpJpAddr::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpCallAddr::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpSeVxByte::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpSneVxByte::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	if (cpu->reg[getX(opcode)] != (opcode & 0x00FF))
		cpu->pc += 2;
}
...
void OpSeVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdVxByte::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpAddVxByte::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpOrVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->reg[getX(opcode)] |= cpu->reg[getY(opcode)];
}
...
void OpAndVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->reg[getX(opcode)] &= cpu->reg[getY(opcode)];
}
...
void OpXorVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->reg[getX(opcode)] ^= cpu->reg[getY(opcode)];
}
...
void OpAddVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpSubVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	uint16_t x = getX(opcode);
	uint16_t y = getY(opcode);
...
void OpShrVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	uint16_t x = getX(opcode);
	uint16_t y = getY(opcode);
...
void OpSubnVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	uint16_t x = getX(opcode);
	uint16_t y = getY(opcode);
...
void OpShlVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	uint16_t x = getX(opcode);
...
void OpSneVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	if (cpu->reg[getX(opcode)] != cpu->reg[getY(opcode)])
		cpu->pc += 2;
}
...
void OpLdIAddr::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->I = opcode & 0x0FFF;
}
...
void OpJpV0Addr::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpRndVxByte::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpDrwVxVy::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) 
{
	uint16_t x = getX(opcode);
	uint16_t y = getY(opcode);
	...
	for (int i = 0; i < height; i++) 
	{
		...
		for (int j = 0; j < 8; j++) 
		{
			if ((pixel & (0x80 >> j)) != 0) 
			{
				Pixel & pixel = screen->getPixel(cpu->reg[x] + j, cpu->reg[y] + i);
...
void OpSkpVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpSknpVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdVxDt::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdVxK::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	bool found = false;
...
void OpLdDtVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdStVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpAddIVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdFVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdBVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	uint16_t x = getX(opcode);
...
void OpLdIVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLdVxI::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	for (uint16_t i = 0; i <= getX(opcode); i++)
...
void OpSys::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
}
...
void OpDown::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {}

string OpDown::disassemble(const uint16_t opcode) {
	return "down";
}

void OpRight::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {}

string OpRight::disassemble(const uint16_t opcode) {
	return "right";
}

void OpLeft::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpLow::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpHigh::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
...
void OpExit::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
	cpu->shutdown();
}
...
void OpLdHFVx::execute(const uint16_t opcode, Cpu * cpu, GameScreen * screen) {
